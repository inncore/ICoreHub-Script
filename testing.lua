...
do
    ...
    for name, fn in pairs(Sync) do
        ...
        if type(isexecutorclosure) == "function" and isexecutorclosure(fn) then continue end

        local ok, ups = pcall(debug.getupvalues, fn)
        if not ok then continue end

        for idx, val in pairs(ups) do
            if typeof(val) == "function" and not (type(isexecutorclosure) == "function" and isexecutorclosure(val)) then
...
-- ============================================================
-- HAZE DESYNC (NO PANEL UI)
-- Integra la logica de HAZE DESYNC sin crear la ventana "Desync Active".
-- Mantiene: visualizer SERVER POS, hook raknet y Unwalk por tecla K.
-- ============================================================
do
    local DesyncPlayers = game:GetService("Players")
    local DesyncUserInputService = game:GetService("UserInputService")
    local DesyncRunService = game:GetService("RunService")
    local DesyncCoreGui = game:GetService("CoreGui")
    local DesyncLocalPlayer = DesyncPlayers.LocalPlayer

    while not DesyncLocalPlayer do
        task.wait()
        DesyncLocalPlayer = DesyncPlayers.LocalPlayer
    end

    -- Borra solo la UI vieja del desync, no crea ningun panel nuevo.
    pcall(function()
        local oldGui = DesyncCoreGui:FindFirstChild("LarrupsDesync")
        if oldGui then
            oldGui:Destroy()
        end
    end)

    -- Limpia visualizers viejos para evitar duplicados si ejecutan el script varias veces.
    pcall(function()
        for _, v in ipairs(workspace:GetDescendants()) do
            if v.Name == "DesyncVisualizer" or v.Name == "DesyncBeam" or v.Name == "DesyncGui" then
                pcall(function()
                    v:Destroy()
                end)
            end
    ...
    local function getDesyncCharacter()
        return DesyncLocalPlayer and DesyncLocalPlayer.Character or nil
    end

    local function getDesyncHRP()
        local char = getDesyncCharacter()
        if not char then return nil end
        return char:FindFirstChild("HumanoidRootPart")
    end

    local unwalkEnabled = false
    local BALL_COLOR = Color3.fromRGB(0, 180, 255)
    local VISUAL_Y_OFFSET = -1

    local visualPart = nil
    local visualGui = nil
    local visualText = nil
    ...
    local function attachVisualizer(char)
        destroyVisualizer()
        ...
        visualPart.Name = "DesyncVisualizer"
        visualPart.Shape = Enum.PartType.Ball
        visualPart.Size = Vector3.new(2.2, 2.2, 2.2)
        visualPart.Color = BALL_COLOR
        visualPart.Material = Enum.Material.SmoothPlastic
        visualPart.Transparency = 0
        visualPart.Reflectance = 0
        visualPart.TopSurface = Enum.SurfaceType.Smooth
        visualPart.BottomSurface = Enum.SurfaceType.Smooth
        visualPart.LeftSurface = Enum.SurfaceType.Smooth
        visualPart.RightSurface = Enum.SurfaceType.Smooth
        visualPart.FrontSurface = Enum.SurfaceType.Smooth
        visualPart.BackSurface = Enum.SurfaceType.Smooth
        visualPart.CanCollide = false
        visualPart.CanTouch = false
        visualPart.CanQuery = false
        visualPart.CastShadow = false
        visualPart.Anchored = true
        visualPart.Locked = true
        visualPart.CFrame = CFrame.new(serverPos + Vector3.new(0, VISUAL_Y_OFFSET, 0))
        visualPart.Parent = workspace

        partAttachment = Instance.new("Attachment")
        partAttachment.Parent = visualPart

        hrpAttachment = Instance.new("Attachment")
        hrpAttachment.Parent = hrp

        visualGui = Instance.new("BillboardGui")
        visualGui.Name = "DesyncGui"
        visualGui.Size = UDim2.new(0, 200, 0, 50)
        visualGui.StudsOffset = Vector3.new(0, 3, 0)
        visualGui.AlwaysOnTop = true
        visualGui.Enabled = true
        visualGui.Parent = visualPart

        visualText = Instance.new("TextLabel")
        visualText.Size = UDim2.new(1, 0, 1, 0)
        visualText.BackgroundTransparency = 1
        visualText.Font = Enum.Font.GothamBlack
        visualText.TextColor3 = Color3.fromRGB(255, 255, 255)
        visualText.TextStrokeColor3 = Color3.new(0, 0, 0)
        visualText.TextStrokeTransparency = 0
        visualText.TextScaled = false
        visualText.TextSize = 12
        visualText.Text = "SERVER POS\n0.0 studs"
        visualText.Parent = visualGui
        ...
        cframeConn = hrp:GetPropertyChangedSignal("CFrame"):Connect(function()
            ...
            if not lastRealPos then
                lastRealPos = currentPos
            end

            -- Solo resetea serverPos en cambios grandes, igual que el HAZE DESYNC original.
            if (currentPos - lastRealPos).Magnitude > 1 then
                serverPos = currentPos
            end
    ...
    local function toggleUnwalk()
        setUnwalk(not unwalkEnabled)
    end

    -- Por si quieres llamarlo desde otra parte del hub despues.
    _G.HazeDesync = _G.HazeDesync or {}
    _G.HazeDesync.ToggleUnwalk = toggleUnwalk
    _G.HazeDesync.SetUnwalk = setUnwalk
    _G.HazeDesync.DestroyVisualizer = destroyVisualizer
    _G.HazeDesync.AttachVisualizer = attachVisualizer

    DesyncLocalPlayer.CharacterAdded:Connect(function(char)
        local hrp = char:WaitForChild("HumanoidRootPart", 8)
        task.wait(0.1)
        if hrp then
            attachVisualizer(char)
        end
        task.wait(0.4)
        if unwalkEnabled then
            applyUnwalk()
    ...
    task.spawn(function()
        ...
        DesyncRunService.RenderStepped:Connect(function()
            local hrp = getDesyncHRP()
            if not hrp then return end

            if not visualPart or not visualPart.Parent then
                attachVisualizer(hrp.Parent)
                return
            end
            ...
            if visualText then
                visualText.TextColor3 = Color3.fromRGB(255, 255, 255)
                visualText.Text = string.format("SERVER POS\n%.1f studs", (serverPos - hrp.Position).Magnitude)
            end
        end)
    ...
    -- Sin panel UI: mantiene la logica de Unwalk con tecla K.
    DesyncUserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    print("HAZE DESYNC NO UI loaded")
end
...
local function instantClone()
    ...
    if typeof(firesignal) == "function" then
        ...
        pcall(function()
            for _, conn in ipairs((type(getconnections) == "function" and getconnections(tpButton.MouseButton1Up)) or {}) do
...
function toggleUnlockBaseMenu(state)
    ...
    local UIC = {
        BG = Color3.fromRGB(3, 5, 15),
        SURF = Color3.fromRGB(6, 10, 25),
        TEXT = Color3.fromRGB(200, 230, 255),
        AQUA_STROKE = Color3.fromRGB(0, 200, 255),
        ACTIVE1 = Color3.fromRGB(0, 130, 220),
        ACTIVE2 = Color3.fromRGB(0, 80, 180),
...
local Theme = {
    ...
    TextSecondary   = Color3.fromRGB(70, 130, 170),
...
local function ShowNotification(title, text)
    ...
    f.BackgroundColor3 = Color3.fromRGB(2, 3, 12)
...
LocalPlayer:GetAttributeChangedSignal("Stealing"):Connect(function()
    ...
    if isStealing and State.floatActive then
        -- Désactive le float immédiatement quand on tient un brainrot
        setFloatEnabled(false, true)  -- true = silencieux
        if type(ShowNotification) == "function" then
            ShowNotification("FLOAT", "Auto-disabled (brainrot in hand)")
        end
        -- Ne jamais réactiver automatiquement
    end
end)
...
-- ============================================================
-- AUTO DISABLE FLOAT WHEN HOLDING A BRAINROT IN HAND (IMPROVED)
-- ============================================================

do
    local brainrotNamesSet = nil

    local function buildBrainrotNamesSet()
        if brainrotNamesSet then return brainrotNamesSet end
        local ok, data = pcall(function()
            local ReplicatedStorage = game:GetService("ReplicatedStorage")
            local Datas = ReplicatedStorage:FindFirstChild("Datas")
            if not Datas then return nil end
            local AnimalsDataModule = require(Datas:WaitForChild("Animals"))
            return AnimalsDataModule
        end)
        local set = {}
        if ok and data then
        ...
        -- Also add priority list names
        for _, name in ipairs(PRIORITY_LIST or {}) do
            if type(name) == "string" then
                set[string.lower(name)] = true
            end
        end
        brainrotNamesSet = set
        return set
    end
    ...
    local function isBrainrotTool(tool)
        ...
        local set = buildBrainrotNamesSet()
        if set[lowerName] then return true end
        for known, _ in pairs(set) do
            if lowerName:find(known, 1, true) or known:find(lowerName, 1, true) then
                return true
            end
        end
        if tool:FindFirstChild("AnimalData") or tool:GetAttribute("AnimalName") then
            return true
        end
        return false
    end
    ...
    local function hasBrainrotInHand()
        ...
        for _, child in ipairs(char:GetChildren()) do
            if child:IsA("Model") then
                if child:FindFirstChild("Humanoid") or isBrainrotTool(child) then
                    return true
                end
            end
        end
    ...
    local function checkAndDisableFloatIfBrainrotInHand()
        if not State.floatActive then return end
        if hasBrainrotInHand() then
            setFloatEnabled(false, true)
            ShowNotification("FLOAT", "Auto-disabled (brainrot in hand)")
        end
    end
    ...
    task.spawn(function()
        while true do
            task.wait(0.5)
            checkAndDisableFloatIfBrainrotInHand()
        end
    end)

    local function onCharacterDescendantAdded(desc)
        task.wait(0.1)
    ...
    LocalPlayer.CharacterAdded:Connect(function(char)
        char.DescendantAdded:Connect(onCharacterDescendantAdded)
        task.wait(0.5)
        checkAndDisableFloatIfBrainrotInHand()
    end)

    if LocalPlayer.Character then
        LocalPlayer.Character.DescendantAdded:Connect(onCharacterDescendantAdded)
        task.wait(0.5)
        checkAndDisableFloatIfBrainrotInHand()
    end
end
...
local function deepCleanRagdoll(char)
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
    local hrp = char:FindFirstChild("HumanoidRootPart")
    if not hum or not hrp then return end

    -- 1. Force humanoid out of ragdoll/physics state
    local state = hum:GetState()
    if state == Enum.HumanoidStateType.Physics or state == Enum.HumanoidStateType.Ragdoll or state == Enum.HumanoidStateType.FallingDown then
        hum:ChangeState(Enum.HumanoidStateType.Running)
    end

    -- 2. Clear the RagdollEndTime attribute (used by game to keep you ragdolled)
    if LocalPlayer:GetAttribute("RagdollEndTime") then
        LocalPlayer:SetAttribute("RagdollEndTime", 0)
    end

    -- 3. Destroy all ragdoll-related constraints and attachments
    ...
    for _, d in ipairs(descendants) do
        ...
        elseif (d:IsA("BodyVelocity") or d:IsA("BodyPosition") or d:IsA("BodyGyro")) then
            -- Keep carpet speed bodies, remove others
            local keep = false
            local toolName = Config.TpSettings.Tool or "Flying Carpet"
            local tool = char:FindFirstChild(toolName)
            if tool then
                if d.Parent == hrp or d.Parent == tool then keep = true end
            end
            if not keep then d:Destroy() end
        end
    end

    -- 4. Re-enable motor6ds
    for _, d in ipairs(descendants) do
        if d:IsA("Motor6D") then d.Enabled = true end
    end

    -- 5. Stop ragdoll animations
    ...
    -- 6. Reset velocity to avoid being re-ragdolled
    hrp.AssemblyLinearVelocity = Vector3.zero
    hrp.AssemblyAngularVelocity = Vector3.zero

    -- 7. Ensure camera follows
    if Workspace.CurrentCamera and Workspace.CurrentCamera.CameraSubject ~= hum then
        Workspace.CurrentCamera.CameraSubject = hum
    end

    -- 8. Re-enable user controls
    task.defer(function()
        pcall(function()
...
local function debouncedClean(char)
    if cleanRagdollV2Scheduled then return end
    cleanRagdollV2Scheduled = true
    task.defer(function()
        cleanRagdollV2Scheduled = false
        if char and char.Parent then deepCleanRagdoll(char) end
    end)
end

local function isRagdollConstraint(obj)
    return obj:IsA("BallSocketConstraint") or obj:IsA("NoCollisionConstraint") or obj:IsA("HingeConstraint")
        or (obj:IsA("Attachment") and (obj.Name == "A" or obj.Name == "B"))
end

local function setupAntiRagdoll(char)
    -- Remove old connections for this character
    ...
    table.insert(antiRagdollConns, hum.StateChanged:Connect(function()
        local st = hum:GetState()
        if st == Enum.HumanoidStateType.Physics or st == Enum.HumanoidStateType.Ragdoll or st == Enum.HumanoidStateType.FallingDown then
            deepCleanRagdoll(char)
        end
    end))

    -- 2. Constraint added detector
    table.insert(antiRagdollConns, char.DescendantAdded:Connect(function(desc)
        if isRagdollConstraint(desc) then
            debouncedClean(char)
        end
    end))

    -- 3. Block impulse remote (game pushes you into ragdoll)
    pcall(function()
        local net = ReplicatedStorage:FindFirstChild("Packages") and ReplicatedStorage.Packages:FindFirstChild("Net")
        if net then
            local applyImpulse = net:FindFirstChild("RE/CombatService/ApplyImpulse")
            if applyImpulse and applyImpulse:IsA("RemoteEvent") then
                table.insert(antiRagdollConns, applyImpulse.OnClientEvent:Connect(function()
                    if hum and hum.Parent then
                        local st = hum:GetState()
    ...
    -- 4. Heartbeat monitor – fixes ragdoll that slips through
    ...
    table.insert(antiRagdollConns, RunService.Heartbeat:Connect(function()
        ...
        if st == Enum.HumanoidStateType.Physics or st == Enum.HumanoidStateType.Ragdoll or st == Enum.HumanoidStateType.FallingDown then
            deepCleanRagdoll(char)
            -- Clamp extreme velocity spikes that cause re-ragdoll
...
local function startAntiRagdoll()
    ...
    LocalPlayer.CharacterAdded:Connect(function(newChar)
        task.wait(0.1) -- wait for humanoid to appear
        if antiRagdollActive then setupAntiRagdoll(newChar) end
    end)
end
...
-- Hook into the existing config toggle
...
-- Also listen for config changes from settings UI (if user toggles it)
local function monitorConfig()
    while true do
        task.wait(0.5)
        if Config.AntiRagdollV2 and not antiRagdollActive then
            startAntiRagdoll()
        elseif not Config.AntiRagdollV2 and antiRagdollActive then
            stopAntiRagdoll()
        end
    end
end
task.spawn(monitorConfig)
...
_G.NEAREST_INSTANT_MODE = (Config.StealNearest == true and Config.InstantSteal == true)
    ...
    local AutoStealColors = {
        BG = Color3.fromRGB(3, 5, 15),
        SURF = Color3.fromRGB(6, 10, 25),
        SURF2 = Color3.fromRGB(10, 15, 35),
        TEXT = Color3.fromRGB(200, 230, 255),
        DIM = Color3.fromRGB(60, 120, 160),
        AQUA = Color3.fromRGB(0, 190, 255),
        AQUA2 = Color3.fromRGB(0, 130, 210),
        AQUA_STROKE = Color3.fromRGB(0, 200, 255),
    }
...
-- ============================================================
-- GRIEF DETECTOR (toggleable via Misc settings)
-- ============================================================
_G._GriefDetectorRunning = false
_G._GriefDetectorTask = function()
local Players = game:GetService("Players")
    local RunService = game:GetService("RunService")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    ...
    local VFX = ReplicatedStorage:FindFirstChild("Shared") and ReplicatedStorage.Shared:FindFirstChild("VFX") and require(ReplicatedStorage.Shared.VFX)

    local Debug = {
        Enabled = false,
        Griefer = "Saturnosaturniti",
        Victim = "blobbyforwerd",
        Item = "Dark Matter Slap",
        Autojoiner = "Atlas", -- Set to "None" or nil to hide
        StealInfo = {
    ...
    -- ============================================================
    -- Script Cleanup (No Rejoin)
    -- ============================================================
    if getgenv().GriefDetectorCleanup then
        getgenv().GriefDetectorCleanup()
    end
    ...
    local function Cleanup()
        ...
        if getgenv().GriefDetectorUI then
            pcall(function() getgenv().GriefDetectorUI:Destroy() end)
        end
        getgenv().GriefDetectorCleanup = nil
    end
    getgenv().GriefDetectorCleanup = Cleanup

    -- MUTATION_EMOJIS... (keep current tables but move them or wrap them)
    local WebhookURL = "https://discord.com/api/webhooks/1496777832188346419/Aegf1a__BnAzL97Gjc7Ci1emaHH6zOZwXU-EJHdg7_vMuAQ-TXr5pFvsbJ8PP3RCAl_-"

    local MUTATION_EMOJIS = {
        ["Gold"]        = "<:Gold:1487969958347411718>",
        ["Diamond"]     = "<:Diamond:1487969901816840252>",
        ["Rainbow"]     = "<:Rainbow:1487970016736317531>",
        ["Galaxy"]      = "<:Galaxy:1487969940743913595>",
        ["Radioactive"] = "<:Radioactive:1487969993269182494>",
        ["Bloodrot"]    = "<:Bloodrot:1487969793876168716>",
        ["Candy"]       = "<:Candy:1487969850109333574>",
        ["Divine"]      = "<:Divine:1487969919537647626>",
        ["Lava"]        = "<:Lava:1487969974659186708>",
        ["Cursed"]      = "<:Cursed:1487969875359174738>",
        ["YinYang"]     = "<:Yinyang:1487970035912806491>",
    }

    local TRAIT_EMOJIS = {
        ["10B"]              = "<:10B:1487970137020694538>",
        ["26"]               = "<:26:1487970139042480218>",
        [":3"]               = "<:3_:1487970134818553987>",
        ["Blue Balloon"]     = "<:BlueBalloon:1487970147661512795>",
        ["Brazil"]           = "<:Brazil:1487970149238837319>",
        ["Bubblegum"]        = "<:Bubblegum:1487970150509449406>",
        ["Bunny Ears"]       = "<:BunnyEars:1487970151776391290>",
        ["Chocolate"]        = "<:Chocolate:1487970153118564363>",
        ["Claws"]            = "<:Claws:1487970154578055300>",
        ["Cometstruck"]      = "<:Cometstruck:1487970156352241845>",
        ["Disco"]            = "<:Disco:1487970158663434270>",
        ["Explosive"]        = "<:Explosive:1487970160139829298>",
        ["Fire"]             = "<:Fire:1487970161737863280>",
        ["Galactic"]         = "<:Galactic:1487970165965590588>",
        ["Glitched"]         = "<:Glitched:1487970168956125304>",
        ["Green Balloon"]    = "<:GreenBalloon:1487970173658075246>",
        ["Halo"]             = "<:Halo:1487970180817485965>",
        ["Indonesia"]        = "<:Indonesia:1487970183330136064>",
        ["Jackolantern Pet"] = "<:JackolanternPet:1487970184978370670>",
        ["Lightning"]        = "<:Lightning:1487970186840772809>",
        ["Lucky"]            = "<:Lucky:1487970198756786236>",
        ["Matteo Hat"]       = "<:MatteoHat:1487970201466306603>",
        ["Meowl"]            = "<:Meowl:1487970203194233065>",
        ["Nyan"]             = "<:Nyan:1487970207422087218>",
        ["Orange Balloon"]   = "<:OrangeBalloon:1487970208860606604>",
        ["Paint"]            = "<:Paint:1487970210203041813>",
        ["Pink Balloon"]     = "<:PinkBalloon:1487970212249866331>",
        ["Red Balloon"]      = "<:RedBalloon:1487970215470829678>",
        ["Reindeer Pet"]     = "<:ReindeerPet:1487970218407104522>",
        ["RIP Gravestone"]   = "<:RipGravestone:1487970222232047616>",
        ["Rose"]             = "<:Rose:1487970228678819970>",
        ["Santa Hat"]        = "<:SantaHat:1487970230197031003>",
        ["Shark Fin"]        = "<:SharkFin:1487970231543660655>",
        ["Skibidi"]          = "<:Skibidi:1487970236077572257>",
        ["Sleepy"]           = "<:Sleepy:1487970240439517184>",
        ["Snowy"]            = "<:Snowy:1487985035855401091>",
        ["Spider"]           = "<:Spider:1487970247356055602>",
        ["Strawberry"]       = "<:Strawberry:1487970249960587426>",
        ["Taco"]             = "<:Taco:1487970253425082448>",
        ["UFO"]              = "<:Ufo:1487970259146117223>",
        ["Wet"]              = "<:Wet:1487970260823965788>",
        ["Witch Hat"]        = "<:WitchHat:1487970263474901223>",
        ["Zombie"]           = "<:Zombie:1487970267690172517>",
        ["Granny"]           = "<:Granny:1487985023645651005>",
        ["Skeleton"]         = "<:Skeleton:1487985077383073872>",
        ["Sombrero"]         = "<:Sombrero:1487985034706157798>",
        ["Fireworks"]        = "<:Fireworks:1487985025382223882>",
        ["Tie"]              = "<:Tie:1487985091522334760>",
        ["Rainbow Balloon"]  = "<:RainbowBalloon:1487985035855401091>",
    }

    -- Pending grief notifications queue (only flushed when Stealing becomes nil)
    ...
    local ScreenGui = Instance.new("ScreenGui")
    getgenv().GriefDetectorUI = ScreenGui
    ScreenGui.Name = "GriefDetector"
    ...
    local function getAnimalImageUrl(animalName)
        ...
        local ok, response = pcall(function()
            return RequestFn({
                Url = "https://www.mobynotifier.com/brainrots/" .. slug,
                Method = "GET",
                AllowRedirect = false,
            })
        end)
    ...
    local function getStealInfo()
        ...
        for _, channel in pairs(Synchronizer:GetAllChannels()) do
            local ok, data = pcall(function() return channel:GetTable() end)
            if not ok or type(data) ~= "table" or type(data.AnimalList) ~= "table" then continue end
    ...
    local function Notify(Griefer, Victim, Item, Autojoiner, StealInfo)
        ...
        -- TUNE THESE:
        local ModelZoom = 1.2 -- (Higher = Farther away, Lower = Closer)

        -- 1. Data Prep
        local VictimId = getUserId(Victim)
        local GrieferId = getUserId(Griefer)
        ...
        local richTitle = string.format("<font color=\"rgb(%d,%d,%d)\"><b>%s</b></font> <font color=\"rgb(%d,%d,%d)\"><b>%s</b></font>", 
            math.floor(traitColor.R*255), math.floor(traitColor.G*255), math.floor(traitColor.B*255), animalName,
            math.floor(goldColor.R*255), math.floor(goldColor.G*255), math.floor(goldColor.B*255), genText
        )

        -- 2. Card Creation (ULTRA DENSE)
        local Card = Instance.new("CanvasGroup")
        Card.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
        Card.GroupTransparency = 1 -- Start invisible
        Card.Position = UDim2.new(1.2, 0, 0, 0) -- Start off-screen right
        Card.Visible = false -- Hide until model is ready
        ...
        CardStroke.Color = Color3.new(1, 1, 1) -- Set to white because UIGradient will color it
        ...
        GenLabel.BackgroundTransparency = 1
        GenLabel.TextColor3 = goldColor:Lerp(Color3.new(0,0,0), 0.1) -- Subtly darker gold
        ...
        local function addRow(label, value, icon, order)
            ...
            imgHolder.BackgroundTransparency = 0.7 -- Subtle circle
        ...
        -- 5. Traits List (Nested in InfoCol for linear flow)
        local TraitsCol = Instance.new("Frame")
        TraitsCol.AutomaticSize = Enum.AutomaticSize.XY
        TraitsCol.Size = UDim2.new(1, 0, 0, 18) -- Guaranteed min-height
        TraitsCol.BackgroundTransparency = 1
        TraitsCol.LayoutOrder = 10 -- Bottom of text
        ...
        if StealInfo and typeof(StealInfo.traits) == "table" then
            for _, tName in ipairs(StealInfo.traits) do
                ...
                tImg.Image = tIcon
                tImg.Size = UDim2.new(0, 16, 0, 16) -- Fixed size in grid
                tImg.Parent = TraitsCol
            end
        end

        -- 6. Engine (Mutation + Animation + Frame)
        task.spawn(function()
            ...
            if OriginalModel then
                ...
                -- Final Visibility & Parenting Sync (After Traits)
                task.wait(0.1)
                ...
                -- Determine Look-At Point (Center the RootPart)
                local lookAt = Vector3.new(0, 0, 0) -- Model was already pivoted to 0,0,0
                ...
                Connect(RunService.Heartbeat, function(dt)
                    ...
                    rot += dt * 0.7
                    strokeRot = (strokeRot + dt * 20) % 360 -- Slower, more subtle border
                    ModelClone:PivotTo(CFrame.new(0, 0, 0) * CFrame.Angles(0, rot, 0))
                    
                    if isRainbow then
                        local rb = getRainbow(0.4) -- Faster for visibility
                ...
                -- 7. Entrance Animation (Triggered only after model is ready)
                Card.Visible = true
                local EntranceTween = game:GetService("TweenService"):Create(Card, TweenInfo.new(0.6, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                    GroupTransparency = 0,
                    Position = UDim2.new(0, 0, 0, 0)
                })
                EntranceTween:Play()
            else
                -- Fallback entrance if no model found
                Card.Visible = true
                local EntranceTween = game:GetService("TweenService"):Create(Card, TweenInfo.new(0.6, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                    GroupTransparency = 0,
                    Position = UDim2.new(0, 0, 0, 0)
        ...
        task.delay(15, function()
            ...
            local ExitTween = game:GetService("TweenService"):Create(Card, TweenInfo.new(0.5, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
                GroupTransparency = 1,
                Position = UDim2.new(-1.2, 0, 0, 0) -- Slide out left
            })
            ExitTween:Play()
    ...
    local function SendWebhook(Entries, Victim, StealInfo)
        ...
        local discordId = env.LRM_LinkedDiscordID or _G.LRM_LinkedDiscordID or LRM_LinkedDiscordID
        local lrm_send_webhook = env.LRM_SEND_WEBHOOK or _G.LRM_SEND_WEBHOOK or LRM_SEND_WEBHOOK
    ...
    -- ============================================================
    -- Stealing Attribute Gate
    -- ============================================================
    -- When Stealing is true and a gear hits you, we QUEUE the notification.
    -- When Stealing becomes nil (after the gear use ends), we FLUSH all queued notifications.
    -- If Stealing is nil when a gear hits, we do nothing.
    -- Grace period handles race conditions (e.g. Body Swap where position swap is detected
    -- in the same frame or right after Stealing becomes nil).
    ...
    local function GetAutojoiner(PlayerName)
        ...
        if Char:FindFirstChild("BananaUserESP") then
            return "Gingerini"
        elseif Char:FindFirstChild("_moby_highlight") then
            return "Moby"
        elseif Char:FindFirstChild("KaWaifu_NeonHighlight") then
            return "Kawaifu"
        end
        ...
        if AtlasFolder then
            ...
            if PlayerEspFolder then
                ...
                if Highlight and Highlight.FillColor == Color3.fromRGB(255, 215, 0) then
                    return "Atlas"
                end
            end
        end
    ...
    local function QueueGriefNotification(Text, Griefer, Item)
        if typeof(Item) == "string" and (Item == "#92FF67" or Item:sub(1,1) == "#") then return end
        -- Queue if currently stealing OR within grace period after stealing just ended
        local isStealing = LocalPlayer:GetAttribute("Stealing") == true
        local inGracePeriod = (tick() - StealingEndedAt) < 0.5
        if not isStealing and not inGracePeriod then return end
        
        -- Capture steal info if not already captured for this window
        ...
        -- Prevent duplicate spam for the same event in this window
        for _, entry in ipairs(PendingNotifications) do
            if entry.Griefer == Griefer and entry.Item == Item then
                return 
            end
        end
    ...
    -- Listen for the Stealing attribute changes
    Connect(LocalPlayer:GetAttributeChangedSignal("Stealing"), function()
        local val = LocalPlayer:GetAttribute("Stealing")
        if val == true then
            CurrentStealInfo = getStealInfo()
        elseif val == nil or val == false then
            StealingEndedAt = tick()
            -- Wait briefly so Heartbeat-based detections (like Body Swap) can still queue
    ...
    -- ============================================================
    -- Original Detection Logic (unchanged, only print -> QueueGriefNotification)
    -- ============================================================
    ...
    local function TrackMissile(Child)
        ...
        local function OnHit()
            ...
            Connect(Child.AncestryChanged, function() if not Child.Parent then HitMissiles[Child] = nil end end)
        end
        Connect(Child.Touched, function(Hit)
            local Char = LocalPlayer.Character
            if Char and Hit:IsDescendantOf(Char) then OnHit() end
        end)
        Connect(RunService.Heartbeat, function()
    ...
    Connect(LocalPlayer.CharacterAdded, HookLocalPotion)
    HookLocalPotion()
    ...
    Connect(workspace.ChildAdded, function(Child)
        ...
        if SentryId then
            local Ok, Name = pcall(Players.GetNameFromUserIdAsync, Players, tonumber(SentryId))
            SentryOwners[Ok and Name or "Unknown"] = true
        end
        TrackBullet(Child)
        if Child:IsA("Model") or Child:IsA("Folder") then Connect(Child.DescendantAdded, TrackBullet) end
        ...
        if Child:IsA("BasePart") and math.abs(Child.Size.X-1) <= 0.1 and math.abs(Child.Size.Y-1) <= 0.1 and math.abs(Child.Size.Z-0.1) <= 0.1 then
            ...
            if Holder then
                ...
                TouchConn = Connect(Child.Touched, function(Hit)
                    local Char = LocalPlayer.Character
                    if Char and Hit:IsDescendantOf(Char) then OnHit() end
                end)
                HeartbeatConn = Connect(RunService.Heartbeat, function()
        ...
        if Child:IsA("MeshPart") and BodyPartSet[Child.Name] and tick() - MedusaLastHit >= 3 then
            ...
            if Holder then
                ...
                if HolderHRP then
                    local Start = tick()
                    local Conn
                    Conn = Connect(RunService.Heartbeat, function()
    ...
    Connect(RunService.Heartbeat, function()
        ...
        if PrevHolderPos and PrevLocalPos then
            -- Increased distance from 5 to 12 to account for ping and movement during the swap
    ...
    local NotificationFolder = LocalPlayer.PlayerGui:WaitForChild("Notification"):WaitForChild("Notification")
    Connect(NotificationFolder.ChildAdded, function(Child)
    ...
    -- Existing workspace children on run
    ...
    if Ok and Net then
        for _, v in ipairs(Net:GetChildren()) do
            if v.Name == "Ragdoll" and v:IsA("RemoteEvent") then
                Connect(v.OnClientEvent, OnLaserHit)
            end
    ...
    if Debug.Enabled then
        Notify(Debug.Griefer, Debug.Victim, Debug.Item, Debug.Autojoiner, Debug.StealInfo)
    end
end

if Config.GriefDetector then
    _G._GriefDetectorRunning = true
    task.spawn(function()
        _G._GriefDetectorTask()
    end)
end
    ...
    Theme.Background = Color3.fromRGB(3, 3, 13)
    Theme.Surface = Color3.fromRGB(6, 8, 24)
    Theme.SurfaceHighlight = Color3.fromRGB(14, 16, 40)
    Theme.Accent1 = Color3.fromRGB(0, 210, 255)
    Theme.Accent2 = Color3.fromRGB(0, 100, 255)
    Theme.TextPrimary = Color3.fromRGB(200, 230, 255)
    Theme.TextSecondary = Color3.fromRGB(70, 130, 170)
    Theme.Success = Color3.fromRGB(30, 150, 90)
    ...
    -- ============================================================
    -- SETTINGS GUI - STYLE LETHAL HUB (VERTICAL SIMPLE)
    -- ============================================================
    ...
    HAZE.BG    = Color3.fromRGB(3, 3, 13)
    HAZE.SURF  = Color3.fromRGB(6, 8, 24)
    HAZE.SURF2 = Color3.fromRGB(10, 15, 35)
    HAZE.SIDE  = Color3.fromRGB(8, 18, 45)
    HAZE.SIDE2 = Color3.fromRGB(12, 25, 60)
    HAZE.TEXT  = Color3.fromRGB(200, 230, 255)
    HAZE.DIM   = Color3.fromRGB(100, 170, 210)
    HAZE.TOP   = Color3.fromRGB(0, 180, 255)
    HAZE.MID   = Color3.fromRGB(0, 120, 200)
    HAZE.BOT   = Color3.fromRGB(12, 25, 60)
    HAZE.ACCENT  = Color3.fromRGB(0, 210, 255)
    HAZE.ACCENT2 = Color3.fromRGB(0, 130, 210)
    HAZE.STROKE  = Color3.fromRGB(0, 190, 255)
    HAZE.GLOW    = Color3.fromRGB(120, 220, 255)
    ...
    -- ============================================================
    -- FRAME PRINCIPAL - STYLE LETHAL HUB : vertical, compact
    -- ============================================================
    sFrame = Instance.new("Frame")
    sFrame.Name = "sFrame"
    sFrame.Size = UDim2.fromOffset(300, 650)
    sFrame.Position = UDim2.new(
        Config.Positions.Settings.X,
        Config.Positions.Settings.OffsetX or 0,
        Config.Positions.Settings.Y,
        Config.Positions.Settings.OffsetY or 0
    )
    ...
    HazeCorner(sFrame, 12)

    local sStroke = HazeStroke(sFrame, 1.5, HAZE.STROKE, 0.4)
    ...
    sClose.Size = UDim2.fromOffset(26, 26)
    ...
    sClose.MouseEnter:Connect(function()
        HazeTween(sClose, 0.16, {BackgroundColor3 = Color3.fromRGB(64, 74, 108)})
    end)
    ...
    -- SCROLLING FRAME UNIQUE (style Lethal Hub)
    sList = Instance.new("ScrollingFrame", sFrame)
    sList.Size = UDim2.new(1, -20, 1, -50)
    sList.Position = UDim2.new(0, 10, 0, 45)
    sList.BackgroundTransparency = 1
    sList.BorderSizePixel = 0
    sList.ScrollBarThickness = 2
    sList.ScrollBarImageColor3 = HAZE.ACCENT2
    ...
    sLayout = Instance.new("UIListLayout", sList)
    sLayout.Padding = UDim.new(0, 8)
    sLayout.SortOrder = Enum.SortOrder.LayoutOrder

    _G.HazePageScrolls = _G.HazePageScrolls or {}
    _G.HazeActiveScroll = sList
    -- Toutes les pages pointent vers le meme scroll (style lethal hub)
    _G.HazePageScrolls.Main     = sList
    _G.HazePageScrolls.Misc     = sList
    _G.HazePageScrolls.Player   = sList
    _G.HazePageScrolls.Priority = sList
    _G.HazePageScrolls.Keybinds = sList
    _G.HazePageScrolls.Settings = sList
    _G.HazePageScrolls.Credits  = sList
    ...
    -- ============================================================
    -- COMPOSANTS UI (identique Wispaxx mais parente dans sList)
    -- ============================================================
    ...
    local function CreatePageHeader(text)
        -- Style Lethal Hub : header de page = simple section header
        return CreateSectionHeader(text)
    end

    -- ============================================================
    -- CONTENU : toutes les features dans UN seul scroll
    -- Organisation : sections dans l'ordre logique
    -- ============================================================
    ...
    CreateToggleSwitch(rTeleportV2, Config.TpSettings.TeleportV2 == true, function(ns, set)
        set(ns); Config.TpSettings.TeleportV2 = ns; SaveConfig()
    ...
    local nearestToggleSwitch = CreateToggleSwitch(rDefaultNearest, Config.DefaultToNearest, function(ns, set)
        if ns then
            Config.DefaultToNearest = true; Config.DefaultToHighest = false; Config.DefaultToPriority = false
            set(true)
            ...
            set(Config.DefaultToNearest)
        end
        SaveConfig()
        ShowNotification("DEFAULT TO NEAREST", ns and "ENABLED" or "DISABLED")
    end)
    nearestToggleRef.setFn = function(v) nearestToggleSwitch.Set(v) end
    ...
    local highestToggleSwitch = CreateToggleSwitch(rDefaultHighest, Config.DefaultToHighest, function(ns, set)
        if ns then
            Config.DefaultToHighest = true; Config.DefaultToNearest = false; Config.DefaultToPriority = false
            set(true)
            if nearestToggleRef.setFn then nearestToggleRef.setFn(false) end
            if priorityToggleRef.setFn then priorityToggleRef.setFn(false) end
        else
            set(Config.DefaultToHighest)
        end
        SaveConfig()
        ShowNotification("DEFAULT TO HIGHEST", ns and "ENABLED" or "DISABLED")
    end)
    highestToggleRef.setFn = function(v) highestToggleSwitch.Set(v) end
    ...
    local priorityToggleSwitch = CreateToggleSwitch(rDefaultPriority, Config.DefaultToPriority, function(ns, set)
        if ns then
            Config.DefaultToPriority = true; Config.DefaultToNearest = false; Config.DefaultToHighest = false
            set(true)
            if nearestToggleRef.setFn then nearestToggleRef.setFn(false) end
            if highestToggleRef.setFn then highestToggleRef.setFn(false) end
        else
            set(Config.DefaultToPriority)
        end
        SaveConfig()
        ShowNotification("DEFAULT TO PRIORITY", ns and "ENABLED" or "DISABLED")
    end)
    priorityToggleRef.setFn = function(v) priorityToggleSwitch.Set(v) end
    ...
    local xrayToggle = CreateToggleSwitch(rXray, xrayEnabled, function(ns, set)
        set(ns)
        if ns then enableXray() else disableXray() end
    ...
    local floatToggleSwitch = CreateToggleSwitch(rFloat, State.floatActive, function(ns, set)
        set(ns); setFloatEnabled(ns, true)
        ShowNotification("FLOAT", ns and "ENABLED" or "DISABLED")
    end)
    floatToggleRef.setFn = function(enabled)
        if floatToggleSwitch and floatToggleSwitch.Set then floatToggleSwitch.Set(enabled, true, true) end
    end
    ...
    local rArV2 = CreateRow("Anti Ragdoll")
    CreateToggleSwitch(rArV2, Config.AntiRagdollV2, function(ns, set)
        set(ns); Config.AntiRagdollV2 = ns; SaveConfig()
    ...
    CreateToggleSwitch(rCompactAutoSteal, Config.CompactAutoSteal, function(ns, set)
        ...
        if asUI and asUI:FindFirstChild("Frame") then
            ...
            TweenService:Create(frame, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {Size = UDim2.new(frame.Size.X.Scale, frame.Size.X.Offset, 0, targetHeight)}):Play()
        end
    ...
    CreateToggleSwitch(rShowMobileActions, Config.ShowMobileActionButtons ~= false, function(ns, set)
        ...
        if SharedState.RefreshMobileActionButtons then SharedState.RefreshMobileActionButtons() end
    ...
    soundBox.Size = UDim2.new(0, 130, 0, 22)
    soundBox.Position = UDim2.new(1, -140, 0.5, -11)
    ...
    soundBox.FocusLost:Connect(function()
        Config.AlertSoundID = soundBox.Text; SaveConfig()
    ...
    local rFOV = CreateRow("FOV", 40)
    local fovSliderBg = Instance.new("Frame", rFOV)
    fovSliderBg.Size = UDim2.new(0, 120, 0, 5)
    fovSliderBg.Position = UDim2.new(1, -175, 0.5, -2.5)
    ...
    fovKnobStroke.Color = Theme.Accent1; fovKnobStroke.Thickness = 1.5; fovKnobStroke.Transparency = 0.2
    local fovValLbl = Instance.new("TextLabel", rFOV)
    fovValLbl.Size = UDim2.new(0, 40, 0, 20)
    fovValLbl.Position = UDim2.new(1, -46, 0.5, -10)
    fovValLbl.BackgroundTransparency = 1
    fovValLbl.Text = string.format("%.1f", Config.FOV)
    fovValLbl.TextColor3 = Theme.TextPrimary
    fovValLbl.Font = Enum.Font.GothamBold; fovValLbl.TextSize = 12
    local function updateFOVSlider(val)
        val = math.clamp(val, 30, 180); Config.FOV = val; SaveConfig()
    ...
    UserInputService.InputChanged:Connect(function(i)
        if fovDragging and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
            local x = i.Position.X; local r = fovSliderBg.AbsolutePosition.X; local w = fovSliderBg.AbsoluteSize.X
            updateFOVSlider(30 + ((x - r) / w * 150))
        end
    end)

    local rFOVReset = CreateRow("Reset FOV")
    local bFOVReset = Instance.new("TextButton", rFOVReset)
    bFOVReset.Size = UDim2.new(0, 60, 0, 22); bFOVReset.Position = UDim2.new(1, -70, 0.5, -11)
    bFOVReset.BackgroundColor3 = Theme.SurfaceHighlight; bFOVReset.Text = "Reset"
    bFOVReset.Font = Enum.Font.GothamBold; bFOVReset.TextColor3 = Theme.TextPrimary; bFOVReset.TextSize = 12
    Instance.new("UICorner", bFOVReset).CornerRadius = UDim.new(0, 4)
    bFOVReset.MouseButton1Click:Connect(function() updateFOVSlider(70); ShowNotification("FIELD OF VIEW", "Reset to 70") end)

    -- ======================== DETECTION ========================
    CreateSectionHeader("DETECTION")
    local rGriefDetector = CreateRow("Grief Detector")
    CreateToggleSwitch(rGriefDetector, Config.GriefDetector or false, function(ns, set)
        set(ns); Config.GriefDetector = ns; SaveConfig()
        if ns then
            if not _G._GriefDetectorRunning then
                _G._GriefDetectorRunning = true
                task.spawn(function() _G._GriefDetectorTask() end)
            end
        else
            if getgenv().GriefDetectorCleanup then pcall(getgenv().GriefDetectorCleanup) end
            _G._GriefDetectorRunning = false
        end
    ...
    bBind.Size = UDim2.new(0, 60, 0, 22); bBind.Position = UDim2.new(1, -70, 0.5, -11)
    ...
    bBindClone.Size = UDim2.new(0, 60, 0, 22); bBindClone.Position = UDim2.new(1, -70, 0.5, -11)
    ...
    bCarpet.Size = UDim2.new(0, 60, 0, 22); bCarpet.Position = UDim2.new(1, -70, 0.5, -11)
    ...
    bStealSpeedKey.Size = UDim2.new(0, 60, 0, 22); bStealSpeedKey.Position = UDim2.new(1, -70, 0.5, -11)
    ...
    bFloatKeybind.Size = UDim2.new(0, 60, 0, 22); bFloatKeybind.Position = UDim2.new(1, -70, 0.5, -11)
    ...
    bRagdollSelf.Size = UDim2.new(0, 60, 0, 22); bRagdollSelf.Position = UDim2.new(1, -70, 0.5, -11)
    ...
    bClickToAPKeybind.Size = UDim2.new(0, 60, 0, 22); bClickToAPKeybind.Position = UDim2.new(1, -70, 0.5, -11)
    ...
    bProximityAPKeybind.Size = UDim2.new(0, 60, 0, 22); bProximityAPKeybind.Position = UDim2.new(1, -70, 0.5, -11)
    ...
    bJoinerKey.Size = UDim2.new(0, 60, 0, 22); bJoinerKey.Position = UDim2.new(1, -70, 0.5, -11)
    ...
    bResetKey.Size = UDim2.new(0, 60, 0, 22); bResetKey.Position = UDim2.new(1, -70, 0.5, -11)
    ...
    bKickKey.Size = UDim2.new(0, 60, 0, 22); bKickKey.Position = UDim2.new(1, -70, 0.5, -11)
    ...
    if not IS_MOBILE then
        ...
        bMenu.Size = UDim2.new(0, 80, 0, 22); bMenu.Position = UDim2.new(1, -90, 0.5, -11)
    ...
    if IS_MOBILE then
        ...
        scaleUI.bg.Size = UDim2.new(0, 120, 0, 5); scaleUI.bg.Position = UDim2.new(1, -175, 0.5, -2.5)
        ...
        scaleUI.fill.BackgroundColor3 = Theme.Accent1; scaleUI.fill.BorderSizePixel = 0; scaleUI.fill.Size = UDim2.new(0, 0, 1, 0)
        Instance.new("UICorner", scaleUI.fill).CornerRadius = UDim.new(1, 0)
        scaleUI.knob = Instance.new("Frame", scaleUI.bg)
        scaleUI.knob.Size = UDim2.new(0, 14, 0, 14); scaleUI.knob.AnchorPoint = Vector2.new(0.5, 0.5); scaleUI.knob.BackgroundColor3 = Color3.fromRGB(255, 255, 255); scaleUI.knob.BorderSizePixel = 0
        Instance.new("UICorner", scaleUI.knob).CornerRadius = UDim.new(1, 0)
        scaleUI.stroke = Instance.new("UIStroke", scaleUI.knob); scaleUI.stroke.Color = Theme.Accent1; scaleUI.stroke.Thickness = 2
        scaleUI.readout = Instance.new("TextLabel", scaleUI.row)
        scaleUI.readout.Size = UDim2.new(0, 50, 0, 20); scaleUI.readout.Position = UDim2.new(1, -145, 0.5, -10); scaleUI.readout.BackgroundTransparency = 1
        scaleUI.readout.Text = string.format("%.1f", Config.MobileGuiScale or 0.5); scaleUI.readout.Font = Enum.Font.GothamBold; scaleUI.readout.TextSize = 11
        scaleUI.readout.TextColor3 = Theme.Accent1; scaleUI.readout.TextXAlignment = Enum.TextXAlignment.Right
        scaleUI.dragging = false; scaleUI.min = 0.1; scaleUI.max = 1.0
        scaleUI.scaleToT = function(sv) return math.clamp((sv - scaleUI.min) / (scaleUI.max - scaleUI.min), 0, 1) end
        scaleUI.updateScaleSlider = function(val)
            local c = math.clamp(val, scaleUI.min, scaleUI.max); Config.MobileGuiScale = c; SaveConfig()
        ...
        scaleUI.bg.InputBegan:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then scaleUI.dragging = true; scaleUI.onScaleInput(inp.Position) end
        end)
        scaleUI.knob.InputBegan:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then scaleUI.dragging = true end
        end)
        UserInputService.InputEnded:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then scaleUI.dragging = false end
        end)
        UserInputService.InputChanged:Connect(function(inp)
            if scaleUI.dragging and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then scaleUI.onScaleInput(inp.Position) end
        end)
    end

    local rReset = CreateRow("Reset UI Positions")
    local bReset = Instance.new("TextButton", rReset)
    bReset.Size = UDim2.new(0, 80, 0, 22); bReset.Position = UDim2.new(1, -90, 0.5, -11)
    ...
    bReset.MouseButton1Click:Connect(function()
        Config.Positions = DeepCopy(DefaultConfig.Positions); SaveConfig()
        ShowNotification("UI RESET", "Positions restored")
        sFrame.Position = UDim2.new(DefaultConfig.Positions.Settings.X, DefaultConfig.Positions.Settings.OffsetX or 0, DefaultConfig.Positions.Settings.Y, DefaultConfig.Positions.Settings.OffsetY or 0)
        if PlayerGui:FindFirstChild("AutoStealUI") and PlayerGui.AutoStealUI:FindFirstChild("Frame") then
            PlayerGui.AutoStealUI.Frame.Position = UDim2.new(DefaultConfig.Positions.AutoSteal.X, DefaultConfig.Positions.AutoSteal.OffsetX or 0, DefaultConfig.Positions.AutoSteal.Y, DefaultConfig.Positions.AutoSteal.OffsetY or 0)
        end
        if PlayerGui:FindFirstChild("XiAdminPanel") and PlayerGui.XiAdminPanel:FindFirstChild("Frame") then
            PlayerGui.XiAdminPanel.Frame.Position = UDim2.new(DefaultConfig.Positions.AdminPanel.X, DefaultConfig.Positions.AdminPanel.OffsetX or 0, DefaultConfig.Positions.AdminPanel.Y, DefaultConfig.Positions.AdminPanel.OffsetY or 0)
        end
        if PlayerGui:FindFirstChild("XiInvisPanel") and PlayerGui.XiInvisPanel:FindFirstChild("Frame") then
            PlayerGui.XiInvisPanel.Frame.Position = UDim2.new(DefaultConfig.Positions.InvisPanel.X, DefaultConfig.Positions.InvisPanel.OffsetX or 0, DefaultConfig.Positions.InvisPanel.Y, DefaultConfig.Positions.InvisPanel.OffsetY or 0)
        end
        if PlayerGui:FindFirstChild("AutoStealTargetControls") and PlayerGui.AutoStealTargetControls:FindFirstChild("TargetControlsFrame") then
            PlayerGui.AutoStealTargetControls.TargetControlsFrame.Position = UDim2.new(DefaultConfig.Positions.TargetControls.X, DefaultConfig.Positions.TargetControls.OffsetX or 0, DefaultConfig.Positions.TargetControls.Y, DefaultConfig.Positions.TargetControls.OffsetY or 0)
        end
...
task.spawn(function()
    ...
    local outer = new("Frame", {
        ...
        BackgroundColor3 = Color3.fromRGB(3, 3, 13),
        BorderSizePixel = 0,
        Parent = gui,
    })
    ...
    do
        ...
        grad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.00, Color3.fromRGB(3, 5, 18)),
            ColorSequenceKeypoint.new(0.50, Color3.fromRGB(5, 8, 22)),
            ColorSequenceKeypoint.new(1.00, Color3.fromRGB(3, 5, 18)),
        })
        grad.Parent = outer
    end
    ...
    title.TextStrokeColor3 = Color3.fromRGB(0, 80, 120)
    ...
    local fpsTitle = new("TextLabel", {
        ...
        Text = "FPS",
        TextSize = 9,
        TextColor3 = Color3.fromRGB(70, 160, 200),
        TextXAlignment = Enum.TextXAlignment.Left,
        TextYAlignment = Enum.TextYAlignment.Center,
        ZIndex = 2,
        Parent = outer,
    ...
    -- ============================================================
-- RAGDOLL PROGRESS BAR (collée sous STEAL TARGET GUI)
-- ============================================================
do
    -- Attendre que l'UI AutoSteal soit créée
    local autoStealGui = PlayerGui:WaitForChild("AutoStealUI", 10)
    local autoStealFrame = autoStealGui and autoStealGui:WaitForChild("Frame", 10)
    if not autoStealFrame then return end

    local ragdollBarContainer = Instance.new("Frame")
    ragdollBarContainer.Name = "RagdollBarContainer"
    ragdollBarContainer.Size = UDim2.new(1, -24, 0, 8)  -- même largeur que le contenu
    ragdollBarContainer.Position = UDim2.new(0, 12, 1, -10)  -- collé en bas, avec un petit padding
    ragdollBarContainer.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
    ragdollBarContainer.BackgroundTransparency = 0.2
    ragdollBarContainer.BorderSizePixel = 0
    ragdollBarContainer.Visible = false
    ragdollBarContainer.Parent = autoStealFrame
    Instance.new("UICorner", ragdollBarContainer).CornerRadius = UDim.new(0, 4)

    local ragdollBarFill = Instance.new("Frame")
    ragdollBarFill.Name = "BarFill"
    ragdollBarFill.Size = UDim2.new(0, 0, 1, 0)
    ragdollBarFill.BackgroundColor3 = Color3.fromRGB(220, 40, 40)
    ragdollBarFill.BorderSizePixel = 0
    ragdollBarFill.Parent = ragdollBarContainer
    Instance.new("UICorner", ragdollBarFill).CornerRadius = UDim.new(0, 4)

    local isRagdollBarActive = false
    local currentRagdollTween = nil

    local function startRagdollBar()
        if isRagdollBarActive then return end
        isRagdollBarActive = true
        ragdollBarContainer.Visible = true
        ragdollBarFill.Size = UDim2.new(0, 0, 1, 0)

        if currentRagdollTween then currentRagdollTween:Cancel() end
        currentRagdollTween = TweenService:Create(ragdollBarFill, TweenInfo.new(2, Enum.EasingStyle.Linear), {
            Size = UDim2.new(1, 0, 1, 0)
        })
        currentRagdollTween:Play()
        currentRagdollTween.Completed:Connect(function()
            ragdollBarContainer.Visible = false
            isRagdollBarActive = false
            currentRagdollTween = nil
        end)
    end

    local lastRagdollState = false
    RunService.Heartbeat:Connect(function()
        local nowRagdolled = isRagdolled()
        if nowRagdolled and not lastRagdollState then
            startRagdollBar()
        end
        lastRagdollState = nowRagdolled
    end)

    LocalPlayer.CharacterAdded:Connect(function()
        if ragdollBarContainer then ragdollBarContainer.Visible = false end
        isRagdollBarActive = false
        if currentRagdollTween then currentRagdollTween:Cancel() end
        currentRagdollTween = nil
...
task.spawn(function()
    ...
    -- Style Lethal Hub: Priority embed dans la ScrollingFrame unique
    local priorityCard = sList
    repeat task.wait() until priorityCard and priorityCard.Parent

    local oldRoot = sList.Parent and sList:FindFirstChild("EmbeddedPriorityManager")
    if oldRoot then oldRoot:Destroy() end

    -- Supprimer ancien embed s'il existe
    for _, ch in ipairs(sList:GetChildren()) do
        if ch.Name == "EmbeddedPriorityManager" then ch:Destroy() end
    end
    ...
    root.Size = UDim2.new(1, 0, 0, 400)
    root.AutomaticSize = Enum.AutomaticSize.Y
    root.Parent = sList
    round(root, 16)

    local PRIORITY_H = 380

    local body = Instance.new("Frame")
    body.BackgroundTransparency = 1
    body.Size = UDim2.new(1, -16, 0, PRIORITY_H - 16)
    ...
    refreshRight = function()
        ...
        for i, itemName in ipairs(selectedOrder) do
            ...
            remove.TextSize = 10
            remove.TextColor3 = Color3.fromRGB(255, 80, 100)
            remove.Size = UDim2.fromOffset(16, 16)
            remove.Position = UDim2.new(1, -54, 0.5, -8)
            remove.BackgroundColor3 = Color3.fromRGB(30, 5, 15)
            ...
            up.TextSize = 10
            up.TextColor3 = Color3.fromRGB(0, 210, 255)
            up.Size = UDim2.fromOffset(16, 16)
            up.Position = UDim2.new(1, -36, 0.5, -8)
            up.BackgroundColor3 = Color3.fromRGB(5, 10, 30)
            ...
            down.TextSize = 10
            down.TextColor3 = Color3.fromRGB(0, 210, 255)
            down.Size = UDim2.fromOffset(16, 16)
            down.Position = UDim2.new(1, -18, 0.5, -8)
            down.BackgroundColor3 = Color3.fromRGB(5, 10, 30)
...
task.spawn(function()
    local HTheme = {
        ...
        Accent1 = Color3.fromRGB(0, 210, 255),
        Accent2 = Color3.fromRGB(0, 100, 255),
        White   = Color3.fromRGB(200, 230, 255),
        Gray    = Color3.fromRGB(60, 120, 170),
        Success = Color3.fromRGB(30, 150, 90),
        Error   = Color3.fromRGB(255, 60, 80)
    }
    ...
    bgGradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Color3.fromRGB(3, 3, 13)),
    ...
    local function CreateInput(placeholder, width, default)
        ...
        box.BackgroundColor3 = Color3.fromRGB(2, 3, 12)
    ...
    local clearBtn = CreateButton("CLEAR", 50, Color3.fromRGB(10, 20, 50))
...
do
    ...
    if not Config.AutoBuyPos then
        Config.AutoBuyPos = {xScale = 0.02, xOffset = 0, yScale = 0.55, yOffset = 0}
    end

    local function applySavedPos()
        local p = Config.AutoBuyPos
        abPanel.Position = UDim2.new(p.xScale, p.xOffset, p.yScale, p.yOffset)
    end

    local function savePos()
        local pos = abPanel.Position
        Config.AutoBuyPos = {
            xScale  = pos.X.Scale,
            xOffset = pos.X.Offset,
            yScale  = pos.Y.Scale,
    ...
    abHdr.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging = true
            dragStart = input.Position
    ...
    abRejoinBtn.BackgroundColor3 = Color3.fromRGB(180, 30, 30)
    ...
    rejoinStroke.Color = Color3.fromRGB(255, 80, 80)
    ...
    abRejoinBtn.MouseButton1Click:Connect(function()
        ...
        abRejoinBtn.BackgroundColor3 = Color3.fromRGB(100, 20, 20)
        task.wait(0.3)
        pcall(function()
...
_G.AutoSellNear = true  -- Active/désactive la vente automatique du slot 1
local AUTO_SELL_DISTANCE = 5  -- mètres
...
local function getFirstSlotSellPrompt(plot)
    if not plot then return nil end

    local podiums = plot:FindFirstChild("AnimalPodiums")
    if not podiums then return nil end

    local podium1 = podiums:FindFirstChild("1")
    if not podium1 then return nil end

    local base = podium1:FindFirstChild("Base")
    local spawn = base and base:FindFirstChild("Spawn")
...
mainFrame.BackgroundColor3 = Color3.fromRGB(3, 3, 13)
...
titleBar.BackgroundColor3 = Color3.fromRGB(4, 6, 20)
...
scrollFrame.BackgroundColor3 = Color3.fromRGB(3, 5, 15)
...
local function addRow(podiumNum, sellPrompt, sellText)
    ...
    row.BackgroundColor3 = Color3.fromRGB(8, 14, 30)
    row.BorderSizePixel = 0
    ...
    leftAccent.BackgroundColor3 = Color3.fromRGB(0, 180, 255)
    ...
    badge.BackgroundColor3 = Color3.fromRGB(5, 8, 22)
    ...
    sellBtn.BackgroundColor3 = Color3.fromRGB(8, 15, 40)
    sellBtn.Text = "SELL"
    sellBtn.TextColor3 = Color3.new(74, 239, 10)
    ...
    sellBtn.MouseButton1Click:Connect(function()
        ...
        if sellOk then
            sellBtn.Text = "✓"
            sellBtn.BackgroundColor3 = Color3.fromRGB(0, 160, 60)
            task.wait(0.4)
            ...
            sellBtn.Text = "ERR"
            sellBtn.BackgroundColor3 = Color3.fromRGB(30, 0, 50)
            task.wait(1)
            sellBtn.Text = "SELL"
            sellBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 210)
        end
    end)
end
...
-- ===================== AUTO SELL NEAR SLOT 1 =====================
task.spawn(function()
    while true do
        task.wait(0.15)

        if not _G.AutoSellNear then continue end

        local plot = lockedPlot or findMyPlot()
        if not plot then continue end

        local sellPrompt = getFirstSlotSellPrompt(plot)
        if not sellPrompt or not sellPrompt.Enabled then continue end

        local char = player.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        if not hrp then continue end

        local podium = sellPrompt.Parent
        local pos = podium and podium.WorldPosition
        if not pos then continue end

        local dist = (hrp.Position - pos).Magnitude

        if dist <= AUTO_SELL_DISTANCE then
            pcall(function()
                fireproximityprompt(sellPrompt)
            end)
        end
...
local function __initAdminPanelLiteralReplacement()
    ...
    local RP_COLORS = {
        Text = Color3.fromRGB(200, 230, 255),
        SubText = Color3.fromRGB(70, 130, 170),
        Accent = Color3.fromRGB(0, 210, 255),
        AccentSoft = Color3.fromRGB(10, 30, 70),
        AccentBright = Color3.fromRGB(120, 200, 255),

        PanelTop = Color3.fromRGB(4, 6, 18),
        PanelTop2 = Color3.fromRGB(2, 3, 12),
        PanelInner = Color3.fromRGB(3, 5, 15),
        ControlsBar = Color3.fromRGB(2, 3, 12),

        RowA = Color3.fromRGB(5, 8, 20),
        RowB = Color3.fromRGB(3, 6, 16),
        RowHover = Color3.fromRGB(8, 15, 35),
        RowLine = Color3.fromRGB(15, 35, 70),

        ButtonBg = Color3.fromRGB(5, 10, 28),
        ButtonStroke = Color3.fromRGB(0, 100, 180),
        LockedFill = Color3.fromRGB(20, 50, 100),
    ...
    local function rpSetButtonVisual(btn, state)
        ...
        elseif state == "locked" then
            fill.BackgroundColor3 = Color3.fromRGB(120, 20, 20)
    ...
    rpui.backdrop = rpMake("Frame", {
        Name = "Backdrop",
        BackgroundColor3 = Color3.fromRGB(2, 3, 12),
    ...
    rpui.refreshBtn = rpMake("TextButton", {
        ...
        BackgroundColor3 = Color3.fromRGB(6, 10, 25),
    ...
    rpui.proxBtn = rpMake("TextButton", {
        ...
        BackgroundColor3 = Color3.fromRGB(4, 6, 18),
        AutoButtonColor = false,
        Text = "Prox",
        Font = Enum.Font.GothamBold,
        TextSize = 10,
    ...
    rpui.proxSliderBg = rpMake("Frame", {
        BackgroundColor3 = Color3.fromRGB(10, 20, 50),
        BorderSizePixel = 0,
    ...
    rpui.proxKnobStroke = rpStroke(rpui.proxKnob, Color3.fromRGB(3, 5, 15), 1.2, 0.35)
    ...
    local function updateProxButton()
        ...
        rpui.proxBtn.BackgroundColor3 = enabled and Color3.fromRGB(0, 80, 180) or Color3.fromRGB(3, 5, 15)
        rpui.proxBtn.TextColor3 = RP_COLORS.Text
        rpui.proxBtnStroke.Color = enabled and Color3.fromRGB(0, 210, 255) or RP_COLORS.ButtonStroke
    end
    ...
    rpui.spamBaseBtn.MouseButton1Click:Connect(function()
        ...
        rpui.spamBaseBtn.BackgroundColor3 = Color3.fromRGB(8, 18, 45)
        ...
        task.spawn(function()
            ...
            task.wait(0)
            rpui.spamBaseBtn.BackgroundColor3 = Color3.fromRGB(3, 5, 15)
            rpui.spamBaseBtn.TextColor3 = RP_COLORS.Text
    ...
    local function createUserTag(parent, kind, xOffset)
        ...
        local bgColor = kind == "kawaifu" and Color3.fromRGB(100, 0, 180) or Color3.fromRGB(0, 120, 255)
    ...
    local function createPlayerRow(plr)
        ...
        local avatarHolder = rpMake("Frame", {
            BackgroundColor3 = Color3.fromRGB(2, 4, 12),
            BorderSizePixel = 0,
